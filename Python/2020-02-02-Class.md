...
If `__new__` is invoked during the object construction, and then `__init__` will be invoked immediately along with `self` passed. If __new__() does not return an instance of cls, then the new instance’s __init__() method will not be invoked.
...
cls. __new__() is a static method. The return value of __new__() should be the new object instance (usually an instance of cls).
...
==There are some weird things, which I am not still clear about==
